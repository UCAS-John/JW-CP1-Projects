...
class Player:
    def __init__(self, health, speed, mana, damage, crit_damage, crit_chance):
        self.health = health
        self.speed = speed
        self.mana = mana
        self.damage = damage
    ...
    def use_item(self, item):
        if item in self.inventory:
            ...
            # Here you would apply the effect of the item to the player.
            print(f"Used {item}")
    ...
    def earn_exp(self, points):
        self.exp += points
        if self.exp >= 100:
            self.upgrade_points += self.exp // 100 * 5
            self.exp = self.exp % 100

    def upgrade_weapon(self, gemstone):
        # Apply gemstone effect to the weapon in the inventory (assumed logic).
...
player = Player(100, 10, 50, 10, 50, 10)
...
while player.dead_count < 3 and timer <= 1800:
    ...
    if action == "explore":
        map_rooms[current_room].explore(player)
    elif action == "rest":
